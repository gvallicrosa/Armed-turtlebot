/*!
  ...
  To calibrate with acquired images
...
//       }
  ...
  else
  {
    ...
    vpXmlParserCamera camSave;
    char camFilename[4096];
    std::string camFilename0 = "tester%d";//opt_ppath + ".xml";
    sprintf(camFilename, camFilename0.c_str(), 0);   
    if(camSave.save(cam, camFilename, "simCam", I.getCols(), I.getHeight()) != vpXmlParserCamera::SEQUENCE_OK)
    std::cout << "Cannot save camera parameters" << std::endl;
